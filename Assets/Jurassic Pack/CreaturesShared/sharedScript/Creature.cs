...
public class Creature : MonoBehaviour
{
	...
	public void SetScale(float resize)
	{
		size = resize;
		speed *= Cretursize;
		transform.localScale = new Vector3(Cretursize, Cretursize, Cretursize); //creature size
		body.mass = baseMass * Cretursize; //creature mass based on size
		...
		source[0].maxDistance = Mathf.Lerp(50f, 300f, Cretursize);
		source[1].maxDistance = Mathf.Lerp(50f, 150f, Cretursize);

	}
	// public void SetScale(float resize)
	// {
	// 	size = resize;
	// 	speed  = speed * Cretursize;
	// 	transform.localScale = new Vector3(Cretursize, Cretursize, Cretursize); //creature size
	// 	body.mass = baseMass * Cretursize; //creature mass based on size
	// 	withersSize = (transform.GetChild(0).GetChild(0).position - transform.position).magnitude; //At the withers altitude
	// 	boxscale = rend[0].bounds.extents; //bounding box scale
	...
	public void ApplyYPos()
	{
		...
		// Check if the animal is clipping through the ground
		CheckGround();
	}
	...
	private void CheckGround()
	{
		...
		RaycastHit hit;
		if (Physics.Raycast(ray, out hit, 1f)) // Raycast to check for ground
		{
	...
	public void Move(Vector3 dir, float force = 0, bool jump = false)
	{
		...
		if (dir != Vector3.zero)
		{
			...
			body.AddForce(dir * force * Cretursize, jump ? ForceMode.VelocityChange : ForceMode.Acceleration);
		}
	}
	#endregion
	...
	public void GetUserInputs(int idle1 = 0, int idle2 = 0, int idle3 = 0, int idle4 = 0, int eat = 0, int drink = 0, int sleep = 0, int rise = 0)
	{
		...
		Vector2 inputVector = rexControls.Rex.Movement.ReadValue<Vector2>();
		...
		if (transform.gameObject == main.creaturesList[main.selected].gameObject && main.cameraMode != 0)
		{
			...
			if (canAttack)
			{
				if (rexControls.Rex.Attack.triggered) { behaviorCount = 500; behavior = "Hunt"; anm.SetBool("Attack", true); }
			...
			//Jump
			//TODO JUMP
			if (canJump && false && !onJump) anm.SetInteger("Move", 3);
			// Move
			else if (inputVector.x != 0 || inputVector.y != 0)
			{
				...
				if (canSwim || (canFly && !isOnGround))
				{
					// if (Input.GetKey(KeyCode.Mouse1))
					// {
					// 	if (verticalInput < 0) anm.SetInteger("Move", -1); // Backward
					// 	else if (verticalInput > 0) anm.SetInteger("Move", 3); // Forward
					// 	else if (horizontalInput > 0) anm.SetInteger("Move", -10); // Strafe-
					// 	else if (horizontalInput < 0) anm.SetInteger("Move", 10); // Strafe+
					// 	else anm.SetInteger("Move", 0);
					// }
					// else
					// {
					// 	anm.SetInteger("Move", run ? 2 : 1); // Swim or fly
					// 	float ang = main.transform.eulerAngles.y + Mathf.Atan2(horizontalInput, verticalInput) * Mathf.Rad2Deg;
				...
				else
				{
					...
					{
						if (IsFacingBackward())  // Check if facing backward
						{
							anm.SetInteger("Move", 1); // Forward walk when facing backward
					...
					// Function to check if the dinosaur is facing backward
					bool IsFacingBackward()
					{
						return transform.forward.z < 0;  // Adjust based on your orientation
					}


					// Calculate the turning angle based on joystick input
					float ang = main.transform.eulerAngles.y + Mathf.Atan2(inputVector.x, inputVector.y) * Mathf.Rad2Deg;
					anm.SetFloat("Turn", ang); // Turn
				}
			}
			// Stop
			...
			else if (rexControls.Rex.Eat.triggered) //Eat / Drink
							//TODO EAT
			{
